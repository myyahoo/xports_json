...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initState() {
    ...
    streamController.stream.listen((p){
      setState(() => newsData.add(p));
    });
    getReq(streamController);
    //this.getReq();
  }
  @override
  Widget build(BuildContext context) {
  ...
  Future<void> getReq(StreamController<NewsArticle> sc) async{
    try {
      print('getreq');
      await Future.delayed(Duration(seconds: 5));
      var client = http.Client();
      // final streamedRes = await client.get('http://www.xportsnews.com/?ac=test&tac=list');
      //var parsed = json.decode(utf8.decode(response.bodyBytes));
      //print(parsed);
      //print(list[10].title);
      var req = new http.Request('get', Uri.parse('http://www.xportsnews.com/?ac=test&tac=list'));
      ...
      streamedRes.stream
          ...
          .expand((e) => e)
          .map((map) => NewsArticle.fromJson(map))
          .pipe(sc);
